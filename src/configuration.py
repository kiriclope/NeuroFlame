...
def init_const(model):
    ...
    model.N_STIM_ON = np.array([int(i / model.DT) + model.N_STEADY for i in model.T_STIM_ON])
    ...
    model.Ka = torch.tensor(model.Ka, dtype=model.FLOAT, device=model.device)
    model.csumNa = torch.cat((torch.tensor([0], device=model.device), torch.cumsum(model.Na, dim=0)))
    ...
    # rate dynamics
    model.TAU = torch.tensor(model.TAU, dtype=model.FLOAT, device=model.device)
    model.EXP_DT_TAU = torch.ones(model.N_NEURON, dtype=model.FLOAT, device=model.device)
    model.DT_TAU = torch.ones(model.N_NEURON, dtype=model.FLOAT, device=model.device)
    ...
    model.EXP_DT_TAU_SYN = torch.ones(model.N_NEURON, dtype=model.FLOAT, device=model.device)
    model.DT_TAU_SYN = torch.ones(model.N_NEURON, dtype=model.FLOAT, device=model.device)

    for i_pop in range(model.N_POP):
        model.EXP_DT_TAU_SYN[model.slices[i_pop]] = torch.exp(-model.DT / model.TAU_SYN[i_pop])
        model.DT_TAU_SYN[model.slices[i_pop]] = model.DT / model.TAU_SYN[i_pop]

    # NMDA dynamics
    if model.IF_NMDA:
        model.TAU_NMDA = torch.tensor(model.TAU_NMDA, dtype=model.FLOAT, device=model.device)
        model.EXP_DT_TAU_NMDA = torch.ones(model.N_NEURON, dtype=model.FLOAT, device=model.device)
        model.DT_TAU_NMDA = torch.ones(model.N_NEURON, dtype=model.FLOAT, device=model.device)

        for i_pop in range(model.N_POP):
            model.EXP_DT_TAU_NMDA[model.slices[i_pop]] = torch.exp(-model.DT / model.TAU_NMDA[i_pop])
    ...
    model.SIGMA = torch.tensor(model.SIGMA, dtype=model.FLOAT, device=model.device).view(model.N_POP, model.N_POP)
    ...
    if 'dual' in model.TASK:
        ...
        if cov_[0, 0] == cov_[0, 1]:
            ...
            model.PHI0 = torch.stack((model.PHI0[0], model.PHI0[0], model.PHI0[1], model.PHI0[1]))
        else:
