...
def train_gng(REPO_ROOT, conf_name, seed, DEVICE):
    ...
    criterion = DualLoss(alpha=0.0, thresh=2.0, rwd_idx=rwd_idx, zero_idx=zero_idx, stim_idx=stim_idx,
                         class_bal=[0.0, 1.0], read_idx=[1, 1])
    ...
    start = perf_counter()
    optimization(model, train_loader, val_loader, criterion, optimizer, zero_grad=0)
    end = perf_counter()
