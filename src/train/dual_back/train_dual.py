...
def train_dual(REPO_ROOT, conf_name, seed, DEVICE):
    ...
    criterion = DualLoss(alpha=0.0, thresh=2.0,
                         stim_idx=stim_idx, gng_idx=gng_idx, cue_idx=cue_idx,
                         rwd_idx=rwd_idx, zero_idx=zero_idx,
...
def test_dual(REPO_ROOT, conf_name, seed, state, DEVICE, IF_OPTO=0, **kwargs):

      N_BATCH = 64

      model = create_model(REPO_ROOT, conf_name, seed, DEVICE, **kwargs)
      path = model.SAVE_PATH
