...
class Plasticity():
    ...
    def __init__(self, USE, TAU_FAC, TAU_REC, DT, size, STP_TYPE='markram', FLOAT=torch.float, device='cuda'):
    ...
    def hansel_stp(self, rates):
        ...
        self.x_stp = self.x_stp - (self.x_stp - 1.0) * self.DT_TAU_REC - self.DT * self.x_stp * self.u_stp * rates
        self.u_stp = self.u_stp - (self.u_stp - self.USE) * self.DT_TAU_FAC + self.DT * self.USE * (1.0 - self.u_stp) * rates
