...
:       5 import gc
:       6 from torch.utils.data import Dataset, TensorDataset, DataLoader
:
...
#+begin_src  ipython
fig, ax = plt.subplots(1, len(tasks), figsize=[len(tasks) * width, width], sharex=1, sharey=1)
...
sep_points = plot_empirical_separatrix(overlaps[-1], ax, window, near_thresh=3)
slope, _ = plot_separatrix_bestfit(sep_points, ax, kind="linear")
ax.set_ylim([-15, 16])
