...
def get_low_rank(rates, model, IF_REC=0):
    if IF_REC==0:
        ...
        try:
            vec3 = model.low_rank.V.T[2]
        except:
            vec3 = model.odors[4] + model.odors[9]

        vec3 = vec3 - (vec3 @ vec2) * vec2 / (vec2 @ vec2)
        # vec1 = vec1 / torch.linalg.norm(vec1)
        # vec2 = vec2 / torch.linalg.norm(vec2)

        vec = torch.stack((vec1, vec2, vec3))
...
def plot_field(overlaps, ax, window, IF_FP=0, task=0, GRID_TEST=0, IF_CBAR=0):
    ...
    heatmap = ax.streamplot(xi.ravel(), yi.ravel(), ui, vi, density=0.5, arrowsize=1.25, norm=norm, color=('w', 0.5))
...
def create_mesh(x, y, size=100):
    ...
    xg = np.linspace(x_min, x_max, size)
    yg = np.linspace(y_min, y_max, size)
...
#+begin_src jupyter-python
def plot_field(overlaps, ax, window, IF_FP=0, task=0, GRID_TEST=0, IF_CBAR=0, IF_STREAM=0):
    x = overlaps[:, window:, 0]
    y = overlaps[:, window:, 1]

    xg, yg, xi, yi, ui, vi = create_mesh(x, y, size=300)
    speed = np.sqrt(ui**2 + vi**2)
    speed = (speed - np.nanmean(speed)) / (np.nanstd(speed) + 1e-6)
    ...
    print("xg", xg.shape, "yg", yg.shape, "ui", ui.shape, "vi", vi.shape)
    print("equal x spacing:", np.allclose(np.diff(xg), np.diff(xg)[0]))
    print("equal y spacing:", np.allclose(np.diff(yg), np.diff(yg)[0]))

    if IF_STREAM:
        ax.streamplot(xg.ravel(), yg.ravel(), ui, vi, density=0.5, arrowsize=1.25, color=(1, 1, 1, 0.5))
    heatmap = ax.pcolormesh(xi, yi, speed, cmap='coolwarm', shading='auto', norm=norm)
...
def plot_field(overlaps, ax, window, IF_FP=0, task=0, GRID_TEST=0, IF_CBAR=0, IF_STREAM=0):
    ...
    # Downsample for quiver so the plot is not overcrowded
    step = 10
    Xq = xi[::step, ::step]
    Yq = yi[::step, ::step]
    Uq = ui[::step, ::step]
...
#+begin_src  jupyter-python
fig, ax = plt.subplots(1, 1, figsize=[1.5 * height, 1.5 * height])
plot_field(overlaps[0], ax, window, IF_FP=0, task=-1, GRID_TEST=model.GRID_TEST, IF_CBAR=0, IF_STREAM=0);
# ax.set_xlim([-.5, .5]);
# ax.set_ylim([-.5, .5]);
plt.show()
