...
#+RESULTS:
:RESULTS:
: EE post 0 pre 0 Jab tensor(0.0750, device='cuda:1') torch.Size([750, 750])
: EE post 0 pre 0 Jab tensor(0.0750, device='cuda:1') torch.Size([750, 750])
# [goto error]
#+begin_example
---------------------------------------------------------------------------
...
Cell In[19], line 3
      1 rates = []
      2 for task in tasks:
----> 3         rates.append(run_grid(GRID_RANGE, seed, task, **kwargs))
      4 rates = np.array(rates)

Cell In[8], line 17, in run_grid(GRID_RANGE, seed, task, **kwargs)
     14 model.N_BATCH = int(model.GRID_SIZE * model.GRID_SIZE)
     16 with torch.no_grad():
---> 17     ff_input = model.init_ff_input()
     18     # print(ff_input.shape, model.N_BATCH)
     20     rates = model(ff_input, RET_REC=0).cpu().detach().numpy()

File ~/models/NeuroFlame/org/train/dual/../../../src/network.py:160, in Network.init_ff_input(self)
    159 def init_ff_input(self):
--> 160     return init_ff_input(self)

File ~/models/NeuroFlame/org/train/dual/../../../src/ff_input.py:274, in init_ff_input(model)
    271     model.VAR_FF.mul_(torch.sqrt(model.Ka[0]))
    272     return init_ff_live(model)
--> 274 return init_ff_seq(model)

File ~/models/NeuroFlame/org/train/dual/../../../src/ff_input.py:223, in init_ff_seq(model)
    220     if "flow" in model.TASK:
    221         del grid_inputs
--> 223 return ff_input * torch.sqrt(model.Ka[0]) * model.M0

OutOfMemoryError: CUDA out of memory. Tried to allocate 804.00 MiB. GPU 1 has a total capacity of 23.60 GiB of which 62.94 MiB is free. Process 71692 has 4.41 GiB memory in use. Process 1273146 has 226.00 MiB memory in use. Process 570398 has 4.11 GiB memory in use. Process 81338 has 4.11 GiB memory in use. Process 528681 has 7.94 GiB memory in use. Including non-PyTorch memory, this process has 2.70 GiB memory in use. Of the allocated memory 2.37 GiB is allocated by PyTorch, and 32.85 MiB is reserved by PyTorch but unallocated. If reserved but unallocated memory is large try setting PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True to avoid fragmentation.  See documentation for Memory Management  (https://pytorch.org/docs/stable/notes/cuda.html#environment-variables)
#+end_example
:END:
...
Cell In[21], line 2
      1 model = init_model(task, seed, **kwargs)
----> 2 overlaps, vec = get_low_rank(rates_tensor, model, IF_REC=0)
      3 print(overlaps.shape)
      5 window = int((model.N_STIM_OFF[0] - model.N_STEADY) / model.N_WINDOW) + 1

Cell In[9], line 12, in get_low_rank(rates, model, IF_REC)
      8     # vec1 = vec1 / torch.linalg.norm(vec1)
      9     # vec2 = vec2 / torch.linalg.norm(vec2)
     11     vec = torch.stack((vec1, vec2))
---> 12     overlaps = rates @ vec.T / model.Na[0]
     13 else:
     14     vec1 = model.low_rank.U.T[0]

RuntimeError: mat1 and mat2 shapes cannot be multiplied (1x0 and 750x2)
#+end_example
:END:
...
: Cell In[24], line 7
:       5     save_fig('flow_field_cbar_seed_%d' % (seed), GRID_TEST=model.GRID_TEST)
:       6 else:
: ----> 7     plot_field(overlaps[i], ax, window, IF_FP=1, task=i, GRID_TEST=model.GRID_TEST, IF_CBAR=0);
:       8     save_fig('flow_field_task_%s_seed_%d' % (tasks[i], seed), GRID_TEST=model.GRID_TEST)
:
...
: NameError                                 Traceback (most recent call last)
: Cell In[25], line 4
:       1 fig, ax = plt.subplots(1, len(tasks), figsize=[len(tasks) * width, width])
:       3 for i in range(len(tasks)):
: ----> 4     plot_field(overlaps[i], ax[i], window, IF_FP=1, task=i, GRID_TEST=model.GRID_TEST)
:       6 save_fig('flow_field_seed_%d' % seed, GRID_TEST=model.GRID_TEST)
:       7 plt.show()
:
: NameError: name 'overlaps' is not defined
