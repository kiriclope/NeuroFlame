...
#+RESULTS:
: training DPA
: Epoch 1/15, Training Loss: 4.3473, Validation Loss: 1.5457, Angle(U, W) : 87.0 °
: Epoch 2/15, Training Loss: 0.4540, Validation Loss: 0.0231, Angle(U, W) : 86.0 °
: Epoch 3/15, Training Loss: 0.0112, Validation Loss: 0.0066, Angle(U, W) : 86.0 °
: Epoch 4/15, Training Loss: 0.0061, Validation Loss: 0.0050, Angle(U, W) : 86.0 °
: Epoch 5/15, Training Loss: 0.0046, Validation Loss: 0.0042, Angle(U, W) : 86.0 °
: Stopping training as loss has fallen below the threshold: 0.0046382684197921594, 0.004224184900522232
: Elapsed (with compilation) = 0h 4m 7s
...
#+RESULTS:
: ff_input torch.Size([4, 605, 1000])
...
#+RESULTS:
: training DRT
: Epoch 1/15, Training Loss: 3.0490, Validation Loss: 0.0442, Angle(U, W) : 88.0 °
: Epoch 2/15, Training Loss: 0.0233, Validation Loss: 0.0058, Angle(U, W) : 87.0 °
: Epoch 3/15, Training Loss: 0.0043, Validation Loss: 0.0037, Angle(U, W) : 87.0 °
: Stopping training as loss has fallen below the threshold: 0.004336761256966453, 0.003662281469782678
...
def plot_overlap_label(readout, y, axis=0, label=['pair', 'unpair'], figname='fig.svg', title='first'):
    ...
    for j in range(3):
        for i in range(2):
            ...
            if axis == 0:
                sign_readout = 2*(y[-1, :, np.newaxis, np.newaxis]-1) * readout
                data = sign_readout[(y[0]==i) & (y[1]==trial[j]), :, axis]
            else:
                data = readout[(y[0]==i) & (y[1]==trial[j]), :, axis]
...
#+begin_src ipython
criterion = DualPerf(thresh=4, cue_idx=cue_idx, rwd_idx=rwd_idx, read_idx=[1, 1])
...
#+begin_example
training Dual
Epoch 1/15, Training Loss: 4.1203, Validation Loss: 2.4283, Angle(U, W) : 81.0 °
Epoch 2/15, Training Loss: 1.2483, Validation Loss: 0.4388, Angle(U, W) : 87.0 °
Epoch 3/15, Training Loss: 0.1797, Validation Loss: 0.0060, Angle(U, W) : 85.0 °
Epoch 4/15, Training Loss: 1.0319, Validation Loss: 3.4745, Angle(U, W) : 85.0 °
Epoch 5/15, Training Loss: 2.0910, Validation Loss: 1.8019, Angle(U, W) : 80.0 °
Epoch 6/15, Training Loss: 1.1769, Validation Loss: 0.8468, Angle(U, W) : 80.0 °
Epoch 7/15, Training Loss: 0.2694, Validation Loss: 0.0049, Angle(U, W) : 80.0 °
Epoch 8/15, Training Loss: 0.0051, Validation Loss: 0.0047, Angle(U, W) : 80.0 °
Epoch 9/15, Training Loss: 0.0048, Validation Loss: 0.0047, Angle(U, W) : 80.0 °
Stopping training as loss has fallen below the threshold: 0.004849008218839, 0.004716155457903038
Elapsed (with compilation) = 0h 5m 35s
#+end_example
...
#+begin_src ipython
criterion = DualPerf(thresh=1.0, cue_idx=cue_idx, rwd_idx=rwd_idx, read_idx=[-1, -1])
...
#+begin_src ipython
print(dpa_perf2.shape, labels.shape)
#+end_src

#+RESULTS:
: torch.Size([384]) torch.Size([384, 3, 31])
