...
# name = 'patients_A'
if IF_LOAD:
    groups = ['controls_long', 'patients_A_long', 'patients_B_long']
else:
    groups = [name]
...
#+RESULTS:
: EE post 0 pre 0 Jab tensor(0.0759, device='cuda:1') torch.Size([750, 750])
...
# model.J_STP = torch.nn.Parameter(kwargs['GAIN_SZ'][0] * model.J_STP.detach())
# model.Wab_T[model.slices[0], model.slices[1]] *= kwargs['GAIN_SZ'][1]
print(model.J_STP)
#+end_src

#+RESULTS:
: Parameter containing:
: tensor(0.9789, device='cuda:1', requires_grad=True)
...
if IF_SAVE_POPT:
    ...
    popt = None
     ...
     model.REP_BIAS = 1.25
     popt = pkl_load('popt_%s' % 'controls', './models/nih/')
...
#+RESULTS:
: torch.Size([768, 1110, 1000]) torch.Size([768, 91, 750])
...
for epoch in tqdm(range(N_TRIALS)):
    with torch.no_grad():
        ...
        # if IF_ITI:
        #     if epoch % interval == 0:
        #         model.T_STEADY = model.ITI_LIST[epoch // interval]

        if model.RANDOM_ITI or model.RANDOM_DELAY:
            init_time_const(model)

        iti_list[epoch] = model.N_STEADY * model.DT + min_iti
...
#+RESULTS:
: saving to ./models/nih//phi_list_patients_C_rand.pkl
: (100, 768, 91)
...
#+RESULTS:
[[file:./figures/odr_seq_sb/figure_42.png]]
...
plt.savefig('./figures/NIH/10_25/acc_delay_%s.svg' % name)
plt.show()
#+end_src

#+RESULTS:
[[file:./figures/odr_seq_sb/figure_45.png]]
...
plt.savefig('./figures/NIH/10_25/sb_stim_hist_%s.svg' % name)
...
:RESULTS:
: [2.] 76800
[[file:./figures/odr_seq_sb/figure_54.png]]
:END:
