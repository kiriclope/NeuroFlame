...
#+RESULTS:
: EE post 0 pre 0 Jab tensor(0.0791, device='cuda:0') torch.Size([750, 750])
...
#+RESULTS:
: torch.Size([768, 920, 1000]) torch.Size([768, 91, 750])
...
#+begin_src ipython
def shifted_phase(phase1, phase2, bias_strength, bias_var, direction=-1, sigma=1):
    """
    ...
    delta = (delta) * torch.pi / 180.0
    phase2_biased = phase2 + direction * bias_strength * torch.sin(2*delta) * torch.exp(-torch.abs(delta)**2 / 2 / sigma**2)  + bias_var * torch.randn_like(phase2)
...
def shifted_phase(phase1, phase2, bias_strength, bias_var, direction='repulsive', sigma=45):
    """
    ...
    # Derivative-of-Gaussian-shaped kernel
    kernel = torch.sin(torch.deg2rad(delta)) * torch.exp(-delta**2 / (2 * sigma**2))
    phase2_biased = (
...
shift =  shifted_phase(phase1, phase2, kwargs['REP_BIAS'], kwargs['REP_VAR'], sigma=60)
plt.plot(rel_loc.cpu().numpy(), (shift-phase2).cpu().numpy(), 'o')
plt.show()
#+end_src
...
iti_list = torch.ones((N_TRIALS+1, model.N_BATCH))
iti_list[0] = model.DURATION - model.T_STIM_OFF[-1]
...
for epoch in tqdm(range(N_TRIALS)):
    with torch.no_grad():
        ...
        iti_list[epoch+1] = model.N_STEADY * model.DT
...
iti_list = np.hstack(iti_list.cpu().numpy()[:-1].T)
...
#+RESULTS:
[[file:./figures/odr_seq_sb/figure_33.png]]
...
#+RESULTS:
[[file:./figures/odr_seq_sb/figure_34.png]]
...
#+RESULTS:
: (76800, 1) (76800, 1) [ -29.999994   39.999992  -65.       -116.000015  120.999985] (100, 768, 91)
...
for i in range(delay_point.shape[1]):
    ...
    ax[0].plot(centers_abs, bin_rel_abs['mean'], color=colors[i])
...
print(delay_point.shape, targ_list.shape, iti_list.shape)
#+end_src

#+RESULTS:
: [2. 4. 8.] 76800
: (76800, 40) (76800, 1) (76800,)
...
for i, iti in enumerate(unique_iti):
    idx = np.where(np.hstack(iti_list) == iti)[0]
    print(len(idx))
    if len(idx) > 0:
