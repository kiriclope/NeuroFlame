...
#+RESULTS:
: (24, 768, 4, 1) (24, 768, 4, 1) torch.Size([768, 1])
...
for i in range(phi_list.shape[0]):
    targ_trial, rel_trial, ref_trial, errors_trial = get_rel_tar_error(phi_list[i, np.newaxis], phi0_list[i, np.newaxis], reference)
...
ax[0].plot(xdelay, serial_list[0], '-')
ax[0].fill_between(xdelay, serial_list[0] - serial_list[1], serial_list[0] + serial_list[1], color='b', alpha=0.2)

ax[0].set_xlabel('Trial #')
ax[0].set_ylabel('Serial Bias (°)')

ax[1].plot(xdelay, ref_list[0], '-')
ax[1].fill_between(xdelay, ref_list[0] - ref_list[1], ref_list[0] + ref_list[1], color='b', alpha=0.2)
...
regressor = AngleDecoder(penalty=None, scaler=True, class_weight=None)
