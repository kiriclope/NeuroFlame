...
#+RESULTS:
:RESULTS:
: 90 345.79127677056186 350.8856143441509
...
def get_tuning_curves(rates, angles):
    ...
    tuning_curve = sum_rates / (angle_counts.unsqueeze(1).float() + 1e-5)
    ...
    return tuning_curve, pref_locs, unique_angles
#+end_src
...
def get_center_tuning_curve(tuning_curve, pref_locs):
    ...
    for i in range(pref_idx.shape[0]):
        center_tuning_curve.append(torch.roll(tuning_curve.T[i], shifts=shifts_[i], dims=0))
...
: Cell In[96], line 2
:       1 if IF_BIAS:
: ----> 2         pkl_save(aligned_angles, 'aligned_angles_bias', path="/home/leon/")
:       3         pkl_save(population_tuning_curve, 'population_tuning_curve_bias', path="/home/leon/")
:       5         aligned_angles_ = pkl_load('aligned_angles', path="/home/leon/")
:
: NameError: name 'aligned_angles' is not defined
:END:
