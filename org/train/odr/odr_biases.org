#+STARTUP: fold
#+TITLE: Biased ODR
#+PROPERTY: header-args:ipython :results both :exports both :async yes :session odr_sb :kernel python3 :exports results :output-dir ./figures/odr_sb :file (lc/org-babel-tangle-figure-filename)
...
:   %reload_ext autoreload
: Python exe
: /home/leon/miniforge3/bin/python
...
#+RESULTS:
[[file:./figures/odr_sb/figure_14.png]]
...
#+RESULTS:
: torch.Size([1, 375]) torch.Size([256, 1])
: torch.Size([1, 375]) torch.Size([256, 1])
: torch.Size([1, 375]) torch.Size([256, 1])
...
#+RESULTS:
:RESULTS:
: 90 305.99998802465825 302.92869784171364
[[file:./figures/odr_sb/figure_17.png]]
:END:
...
:RESULTS:
: tensor(337.)
[[file:./figures/odr_sb/figure_18.png]]
:END:
...
plt.show()
 ...
 #+RESULTS:
 [[file:./figures/odr_sb/figure_20.png]]
...
:RESULTS:
: tensor(89.)
[[file:./figures/odr_sb/figure_24.png]]
:END:
...
#+RESULTS:
[[file:./figures/odr_sb/figure_25.png]]
...
: Cell In[49], line 1
: ----> 1 from astropy.stats.circstats import circmean
:
...
Cell In[50], line 11
      8         if 0 <= bin_index < nbins:  # Ensure index is within valid range
      9             normalized_rates[j] = rate / counts[bin_index] if counts[bin_index] > 0 else 0
---> 11     pref_locs.append(circmean(angles, weights=normalized_rates, axis=0))
     12 pref_locs = np.array(pref_locs)
     13 print(pref_locs.shape)

File ~/miniforge3/lib/python3.12/site-packages/scipy/stats/_axis_nan_policy.py:586, in _axis_nan_policy_factory.<locals>.axis_nan_policy_decorator.<locals>.axis_nan_policy_wrapper(***failed resolving arguments***)
    583     res = _add_reduced_axes(res, reduced_axes, keepdims)
    584     return tuple_to_result(*res)
--> 586 res = hypotest_fun_out(*samples, **kwds)
    587 res = result_to_tuple(res, n_out)
    588 res = _add_reduced_axes(res, reduced_axes, keepdims)

TypeError: circmean() got an unexpected keyword argument 'weights'
#+end_example
:END:
...
Cell In[51], line 10
      8 pref_locs = []
      9 for i in range(mean_rates.shape[1]):
---> 10         pref_locs.append(circmean(angles, weights=normalized_rates[:, i], axis=0))
     12 pref_locs = (np.array(pref_locs) + 2.0 * np.pi) % (2.0 * np.pi)
     13 print(pref_locs.shape, normalized_rates.shape)
...
: Cell In[53], line 1
: ----> 1 theta = np.linspace(0, 360, pref_locs.shape[-1])
:       2 plt.scatter(theta, pref_locs * 180 / np.pi)
:       3 plt.xlabel('Ground Truth (°)')
:
...
#+RESULTS:
[[file:./figures/odr_sb/figure_50.png]]
...
#+RESULTS:
[[file:./figures/odr_sb/figure_51.png]]
...
#+RESULTS:
[[file:./figures/odr_sb/figure_53.png]]
...
#+RESULTS:
[[file:./figures/odr_sb/figure_55.png]]
...
#+RESULTS:
[[file:./figures/odr_sb/figure_57.png]]
